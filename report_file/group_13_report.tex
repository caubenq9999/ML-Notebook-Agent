...
\begin{frame}{Multi-agent Pipeline}
  ...
  \begin{itemize}
    \item \textbf{Curriculum Agent:} Sinh lộ trình học, phù hợp riêng cho từng dạng: Học có giám sát (Supervised) hoặc Học không có giám sát (Unsupervised).
    \item \textbf{Cấu trúc Notebook chuẩn 4 khối:}
      \begin{enumerate}
        ...
        \item \textbf{Chuẩn bị dữ liệu:} Nạp dữ liệu với bộ dataset tương ứng (Heart Failure, Red Wine Quality, Mall Customers) và chèn sẵn code prepocessing, EDA, split train/test(Supervised).
  ...
  \begin{itemize}
    ...
    \item \textbf{Các chỉ số đo lường chính:}
      \begin{itemize}
        ...
        \item \textbf{LLM Judge Agreement (Cohen's Kappa):} Đạt $\kappa \ge 0.70$ (Substantial Agreement), đảm bảo độ đồng thuận cao giữa điểm đánh giá của LLM Judge và chuyên gia (Human Benchmark).
