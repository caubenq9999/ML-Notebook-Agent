    ...
    \begin{column}{0.52\textwidth}
      \begin{block}{Tổng quan dự án \& bối cảnh thực tế}
        \begin{itemize}
          ...
          bị đặt thông tin lý thuyết không chuẩn xác (hallucination).
        \end{itemize}
      \end{block}
    \end{column}
...
% ------------------------------------------------------------
\begin{frame}[fragile]{Đặt vấn đề và giải pháp}
...
%  SLIDE 5 – Multi-agent
% ------------------------------------------------------------
\begin{frame}[fragile]{Multi-agent Pipeline}
...
% ------------------------------------------------------------
%  SLIDE 7 – Research Agent & RAG (ResearchBundle)
% ------------------------------------------------------------
\begin{frame}[fragile]{Multi-agent Pipeline}
  Research Agent: Quy trình RAG \& Đóng gói ResearchBundle
  \begin{itemize}
    \item \textbf{Knowledge Base (.md):} Kho tri thức chuẩn hóa từ tài liệu chính thống (Scikit-Learn User Guide), đóng vai trò "sách giáo khoa" tham chiếu chống bịa đặt (hallucination).
    \item \textbf{Kiến trúc RAG 2 giai đoạn (Two-Stage RAG):}
      \begin{itemize}
        \item \textbf{Giai đoạn 1 (BM25 Retrieval):} Truy xuất chính xác các đoạn văn bản lý thuyết, công thức và code mẫu liên quan từ Knowledge Base.
        \item \textbf{Giai đoạn 2 (LLM Re-ranking):} Tự động sắp xếp, lọc bớt khái niệm dựa trên thời lượng \texttt{duration\_minutes} và trình độ \texttt{level\_final}.
      \end{itemize}
    \item \textbf{Fallback Web Search (Wikipedia API):} Khi Knowledge Base không phủ hết khái niệm, hệ thống gọi Wikipedia API với bộ lọc token hit ratio $\ge 0.5$ để lấy thông tin bổ trợ.
    \item \textbf{Đầu ra - ResearchBundle:} Tổng hợp nội dung đã RAG (Lý thuyết, Code mẫu, Mẹo thực hành) thành một gói thông tin cấu trúc để cung cấp trực tiếp cho Curriculum Agent.
  \end{itemize}
\end{frame}
...
% ------------------------------------------------------------
\begin{frame}[fragile]{Multi-agent Pipeline}
  Curriculum \& Generator Agent: Sinh lộ trình học \& Đóng gói Notebook
  \begin{itemize}
    \item \textbf{Curriculum Agent:} Sinh lộ trình học dựa trên \texttt{ResearchBundle}, phù hợp riêng cho từng dạng: Học có giám sát (Supervised) hoặc Học không có giám sát (Unsupervised).
    \item \textbf{Cấu trúc Notebook chuẩn 4 khối:}
      \begin{enumerate}
        \item \textbf{Tổng quan bài học:} Giới thiệu về Topic và dataset (Tên, Nguồn, Kích thước, Biến mục tiêu, Đặc tả)
        \item \textbf{Chuẩn bị dữ liệu:} Nạp dữ liệu với bộ dataset tương ứng (Heart Failure, Red Wine Quality, Mall Customers) và chèn sẵn code preprocessing, EDA, split train/test (Supervised).
        \item \textbf{Nội dung \& Bài tập thực hành:} Giới thiệu kiến thức nền tảng, code demo giải mẫu bài tập, nêu các yêu cầu bài tập dựa trên lộ trình học.
...
%  SLIDE 10 – Demo
% ------------------------------------------------------------
\begin{frame}[fragile]{Demo \& Evaluation}
...
% ------------------------------------------------------------
%  SLIDE 11 – Evaluation, Benchmark & Ablation Study
% ------------------------------------------------------------
\begin{frame}[fragile]{Demo \& Evaluation}
  Kết quả Kỹ thuật, Benchmark Metrics \& Ablation Study
  \begin{itemize}
    \item \textbf{Thử nghiệm trên 6 Test Cases tiêu biểu:} Nhằm tối ưu hóa chi phí API mà vẫn đảm bảo tính đại diện, 6 bài thử nghiệm được thiết kế phủ đều 3 chủ đề (Logistic Regression, Decision Tree, K-Means) trên 2 cấp độ trình độ khác nhau.
    \item \textbf{Chỉ số đánh giá chính (Evaluation Metrics):}
      \begin{itemize}
        \item \textbf{Execution Pass Rate:} Tỷ lệ Notebook chạy thành công 100\% từ trên xuống dưới, không gặp bất kỳ lỗi Python runtime nào.
        \item \textbf{Final Pass Rate:} Tỷ lệ Notebook vượt qua cả 8 Hard Rules kỹ thuật VÀ đạt điểm LLM Judge $\ge 4/5$.
        \item \textbf{LLM Judge Agreement:} Độ đồng thuận cao với chuyên gia human benchmark ($\kappa \ge 0.70$).
      \end{itemize}
    \item \textbf{Kết quả Ablation Study (Đánh giá vai trò từng thành phần):}
      \begin{itemize}
        \item \textbf{Thiếu RAG ResearchBundle:} Tỷ lệ bịa đặt kiến thức (hallucination) tăng, lý thuyết rời rạc không bám sát chuẩn Scikit-Learn.
        \item \textbf{Thiếu Retry Loop (Sandbox):} Tỷ lệ Execution Pass Rate giảm mạnh do không khắc phục được các lỗi runtime nhỏ phát sinh lúc sinh code.
        \item \textbf{Thiếu Hard Rules / LLM Judge:} Notebook dễ vi phạm lỗi Data Leakage hoặc thiếu cấu trúc bài tập chuẩn sư phạm.
      \end{itemize}
  \end{itemize}
\end{frame}
...
% ------------------------------------------------------------
\begin{frame}[fragile]{Conclusion \& Mentor Q\&A}
  \begin{columns}[T]
