...
class ConceptSelection(BaseModel):
    """Structured output for the shared, cost-tracked LLM client."""
...
def _stage2_llm_filtering(
    ...
    Chỉ trả về JSON object có dạng {{"concepts": ["Khái niệm A", "Khái niệm B"]}}.
    Không giải thích và không chọn khái niệm ngoài danh sách ứng viên.
    """

    try:
        session_id = (
            learner_profile.session_id
            if learner_profile is not None
            else f"research-{resolve_topic_alias(topic)}"
        )
        selection, _usage = call_json(
            prompt,
            schema=ConceptSelection,
            session_id=session_id,
        )
        assessed_concepts = selection.concepts
