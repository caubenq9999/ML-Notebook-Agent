...
def run_research(
    ...
    # 3.5. RAG: TRÍCH XUẤT LÝ THUYẾT THEO SEMANTIC CHUNKING (embedding), gom các concept
    # cùng thuộc 1 đoạn kiến thức lại với nhau -- tái dùng `kb_index` đã build ở bước 1
    # (Sparse Retrieval), KHÔNG build lại lần 2. Nếu topic không có KB (kb_entries rỗng,
    # đi theo nhánh web search) hoặc thiếu thư viện embedding/numpy, hàm này tự trả về []
    # (xem graceful fallback trong tools/kb_reader.py) -- KHÔNG làm crash run_research().
    concept_source_map = {
...
if __name__ == "__main__":
    ...
    raw_topic = input("1. Nhập topic (vd: logistic, dt, k_means) [Mặc định: logistic]: ").strip() or "logistic"

    user_profile = LearnerProfile(
        topic=raw_topic, level_declared=1, level_final=1, quiz_score=3,
