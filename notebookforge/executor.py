...
CELL_TIMEOUT = 120  # giây/cell - đề cương mục 2.3 + 2.8: "timeout 120 giây, tắt mạng"
KERNEL_NAME = "python3"
...
def run_notebook(nb_path: str, attempt: int = 1) -> ExcRes:
    ...
    client = NotebookClient(
        ...
        # "tắt mạng" theo đề cương mục 2.3/2.8: chặn ở tầng biến môi trường,
        # mọi thư viện đi qua requests/urllib đều trượt proxy chết -> fail nhanh.
        # Không phải sandbox thật (code vẫn mở socket trực tiếp được), nhưng chặn
        # được trường hợp hay gặp nhất: LLM sinh code tải dataset từ internet.
        kernel_env=_OFFLINE_ENV,
    )
    ...
    try:
        ...
        timeout_hit = True
        fatal = CellError(
            cell_index=getattr(exc, "cell_index", 0) or 0,
