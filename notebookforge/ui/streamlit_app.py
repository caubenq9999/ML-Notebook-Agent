...
def fallback_result(profile_data: dict) -> tuple[dict, dict] | None:
    """Chỉ dùng mock khi demo bật cờ rõ ràng; mặc định để lộ lỗi backend thật."""
    if ALLOW_MOCK_FALLBACK:
        st.info("🔄 Mock fallback đã được bật bằng biến môi trường.")
        ...
        for _ in range(MAX_RETRIES):
            ...
            if res.status_code == 200:
                ...
                if status in ["completed", "success"]:
                    ...
                    if not report:
                        report = report_data.get("quality_report")
...
if "notebook_dict" in st.session_state and st.session_state.notebook_dict:
    ...
    if isinstance(report_data, dict) and "scores" not in report_data:
        report_data = normalize_report_payload(report_data) or report_data
    if report_data:
        st.divider()
        with st.expander(
            ...
            if isinstance(report_data, dict):
                ...
                attempts_used = report_data.get("attempts_used")
                if attempts_used is not None:
                    st.caption(f"**Số lần sinh đã dùng:** {attempts_used} / 2")
